...
def train():
    ...
    with wandb.init() as run:
        config = run.config
        wandb.config.architecture_name = "TSR-CNN2"
        wandb.config.dataset_name = "GTSRB"

        model = TSRNet2.build_model(config.f1, config.f2, config.f3, config.fc_layer_size, config.drop)
        ...
        model_path = os.path.join('TSRNet2', 'models', f'{run.name}.h5')
...
sweep_id = wandb.sweep(sweep=sweep_configuration, project="TSRNet2")
